"""Merge every migration data source (tools, news, hand-exported blog posts,
freshly crawled blog posts) into one normalized payload that push_to_wp.py
can post to the WordPress REST API without needing to know where each piece
of content originally came from.
...
Requires data/blog_posts_crawled.json to exist (produced by crawl.py) --
run parse_exports.py and crawl.py first.
"""
...
def build_news() -> list[dict]:
    news = json.loads((DATA / "news_seed.json").read_text(encoding="utf-8"))
    out = []
    for n in news:
        body = re.sub(r"^## .+\nURL: .+\n\n", "", n["body_md"], count=1)
